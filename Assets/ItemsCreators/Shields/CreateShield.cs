...
public class CreateShield : MonoBehaviour 
{
	...
	private PlayerItemsManager.ItemObject m_Shield;
	...
	void Start () 
	{
		...
		m_Shield = PlayerItemsManager.ItemObject.CreateArmorTypeItem(PlayerItemsManager.ItemType.ITEM_LHAND, 
			                                                         m_Texture2D, 
			                                                         m_SellValue, 
			                                                         m_Defense, 
			                                                         m_ReqLvl, 
	...
	void Update()
	{
		if(m_ItemWasTakenByPlayer)
		{
			...
			else if(!m_ItemPickedUp)
			{
				...
				m_pPlayerMsgSystemManager.AddMsgToLog("Vous ramassez un bouclier.", PlayerMsgSystemManager.MsgType.MSG_ITEMS);
				renderer.enabled = false; //hide the object untill destruction
				m_pItemsManager.PickupNewItem(ref m_Shield);
				m_ItemPickedUp = true;
			}
		}
	}
