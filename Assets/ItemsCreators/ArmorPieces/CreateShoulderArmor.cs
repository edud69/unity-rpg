...
public class CreateShoulderArmor : MonoBehaviour 
{
	...
	private PlayerItemsManager.ItemObject m_ShoulderArmor;
	...
	void Start () 
	{
		...
		m_ShoulderArmor = PlayerItemsManager.ItemObject.CreateArmorTypeItem(PlayerItemsManager.ItemType.ITEM_SHOULDER, 
			                                                                m_Texture2D, 
			                                                                m_SellValue, 
			                                                                m_Defense, 
			                                                                m_ReqLvl, 
	...
	void Update()
	{
		if(m_ItemWasTakenByPlayer)
		{
			...
			else if(!m_ItemPickedUp)
			{
				...
				m_pPlayerMsgSystemManager.AddMsgToLog("Vous ramassez des epaulettes.", PlayerMsgSystemManager.MsgType.MSG_ITEMS);
				renderer.enabled = false; //hide the object untill destruction
				m_pItemsManager.PickupNewItem(ref m_ShoulderArmor);
				m_ItemPickedUp = true;
			}
		}
	}
