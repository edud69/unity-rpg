...
public class CreateChestArmor : MonoBehaviour 
{
	...
	private PlayerItemsManager.ItemObject m_ChestArmor;
	...
	void Start () 
	{
		...
		m_ChestArmor = PlayerItemsManager.ItemObject.CreateArmorTypeItem(PlayerItemsManager.ItemType.ITEM_CHEST, 
			                                                             m_Texture2D, 
			                                                             m_SellValue, 
			                                                             m_Defense, 
			                                                             m_ReqLvl, 
	...
	void Update()
	{
		if(m_ItemWasTakenByPlayer)
		{
			...
			else if(!m_ItemPickedUp)
			{
				...
				m_pPlayerMsgSystemManager.AddMsgToLog("Vous ramassez un plastron.", PlayerMsgSystemManager.MsgType.MSG_ITEMS);
				renderer.enabled = false; //hide the object untill destruction
				m_pItemsManager.PickupNewItem(ref m_ChestArmor);
				m_ItemPickedUp = true;
			}
		}
	}
