...
public class CreateHelmArmor : MonoBehaviour 
{
	...
	private PlayerItemsManager.ItemObject m_HelmArmor;
	...
	void Start () 
	{
		...
		m_HelmArmor = PlayerItemsManager.ItemObject.CreateArmorTypeItem(PlayerItemsManager.ItemType.ITEM_HELM, 
			                                                            m_Texture2D, 
			                                                            m_SellValue, 
			                                                            m_Defense, 
			                                                            m_ReqLvl, 
	...
	void Update()
	{
		if(m_ItemWasTakenByPlayer)
		{
			...
			else if(!m_ItemPickedUp)
			{
				...
				m_pPlayerMsgSystemManager.AddMsgToLog("Vous ramassez un casque.", PlayerMsgSystemManager.MsgType.MSG_ITEMS);
				renderer.enabled = false; //hide the object untill destruction
				m_pItemsManager.PickupNewItem(ref m_HelmArmor);
				m_ItemPickedUp = true;
			}
		}
	}
