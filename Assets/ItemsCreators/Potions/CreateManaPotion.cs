...
public class CreateManaPotion : MonoBehaviour 
{
	...
	private PlayerItemsManager.ItemObject m_ManaPotion;
	...
	void Start () 
	{
		...
		m_ManaPotion       = PlayerItemsManager.ItemObject.CreatePotion(PlayerItemsManager.ItemType.ITEM_MANAPOTION, m_pTexture, m_IncrementMP, m_SellValue);
	}
	...
	void Update()
	{
		if(m_ItemWasTakenByPlayer)
		{
			...
			else if(!m_ItemPickedUp)
			{
				...
				m_pPlayerMsgSystemManager.AddMsgToLog("Vous ramassez une potion de spirit.", PlayerMsgSystemManager.MsgType.MSG_ITEMS);
				renderer.enabled = false; //hide the object untill destruction
				m_pItemsManager.PickupNewItem(ref m_ManaPotion);
				m_ItemPickedUp = true;
			}
		}
	}
