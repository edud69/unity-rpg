...
public class CreateHealthPotion : MonoBehaviour 
{
	public	int							  m_SellValue   = 0;
	public  float						  m_IncrementHP = 0f;
	...
	private PlayerItemsManager.ItemObject m_HealthPotion;
	...
	void Start () 
	{
		...
		m_HealthPotion = PlayerItemsManager.ItemObject.CreatePotion(PlayerItemsManager.ItemType.ITEM_HEALTHPOTION, m_pTexture, m_IncrementHP, m_SellValue);
	}
	...
	void OnMouseOver()
	{
		if(this.CheckDistance())
		{
			...
			renderer.material.SetColor("_OutlineColor", pColor);
	   		renderer.material.SetFloat("_Outline", 0.003f); //change outline width
		}
	}
	...
	void Update()
	{
		if(m_ItemWasTakenByPlayer)
		{
			...
			else if(!m_ItemPickedUp)
			{
				...
				m_pPlayerMsgSystemManager.AddMsgToLog("Vous ramassez une potion de vitalite.", PlayerMsgSystemManager.MsgType.MSG_ITEMS);
				renderer.enabled = false; //hide the object untill destruction
				m_pItemsManager.PickupNewItem(ref m_HealthPotion);
				m_ItemPickedUp = true;
			}
		}
	}
